...
public class CustomAddFreeRideActivity extends BaseActivity implements LocationSource, AMapLocationListener, GeocodeSearch.OnGeocodeSearchListener {
    ...
    @Bind(R.id.activity_custom_add_free_ride)
    LinearLayout activityCustomAddFreeRide;

    private final static int REQUEST_PLACE = 3;//地址返回结果码
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_custom_add_free_ride);
        ButterKnife.bind(this);
        mMapView.onCreate(savedInstanceState);// 此方法必须重写
        steepStatusBar();
        setMargins(activityCustomAddFreeRide);
        initView();
        initMap();
        initLocation();
    ...
    public void initView() {
        ...
        cusAddFreeView = new CusAddFreeView(mContext);
        cusAddFreeView.setSelectListener(new View.OnClickListener() {
            @Override
        ...
        cusAddFreeView.setListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                isLoc = true;
                if (city != null)
                    startActivityForResult(new Intent(mContext, InputSearchActivity.class)
                            .putExtra("city", city).putExtra("flag", 2), REQUEST_PLACE);
        ...
        cusAddFreeView.setBookListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                if (startLng == 0 || startLat == 0 || startPlace == null) {
                    ToastUtils.showTextToast(mContext, "起点不能为空!");
                    return;
                } else if (endLng == 0 || endLat == 0 || endPlace == null) {
                    ToastUtils.showTextToast(mContext, "终点不能为空!");
                    return;
                } else if (num == 0) {
                    ToastUtils.showTextToast(mContext, "请选择乘车人数!");
                    return;
                } else if (startTime == null) {
                    ToastUtils.showTextToast(mContext, "请选择出发时间!");
                    return;
                }
                startActivity(new Intent(mContext, CusWaitDriverOrdersActivity.class).putExtra("startPlace", startPlace)
                        .putExtra("endPlace", endPlace).putExtra("num", num).putExtra("startTime", startTime));
            }
        });
        rlContentCA.addView(cusAddFreeView);
    }
    ...
    private void showNumDialog() {
        ...
        numDialog.setOnOperItemClickL(new OnOperItemClickL() {
            ...
            public void onOperItemClick(AdapterView<?> parent, View view, int position, long id) {
                num = position;
                cusAddFreeView.setPeopleNum(stringItems[position]);
                numDialog.dismiss();
            }
        });
        numDialog.show();
        ...
        startTimeDialog.setButton(new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialogInterface, int i) {
                if (startTimeDialog.getSelectIndex() != null && startTimeDialog.getSelectIndex().length == 6) {
                    ...
                    cusAddFreeView.setStartTime(selectIndex[3] + selectIndex[4] + selectIndex[5]);
                    startTime = selectIndex[3] + selectIndex[4] + selectIndex[5];
                }
                startTimeDialog.dismiss();
            }
        });
        ...
        if (mListener != null && aMapLocation != null) {
            if (aMapLocation.getErrorCode() == 0) {
                ...
                if (isFirstLoc) {
                    ...
                    cusAddFreeView.setLoc(startPlace);
                    //将地图移动到定位点
        ...
        if (resultCode == InputSearchActivity.RESUALT_PLACE) {
            if (requestCode == REQUEST_PLACE) {
                if (isLoc) {
                    ...
                    cusAddFreeView.setDes(endPlace);
                }
            }
        }
    }
