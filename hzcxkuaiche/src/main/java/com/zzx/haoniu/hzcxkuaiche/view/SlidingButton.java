package com.zzx.haoniu.hzcxkuaiche.view;
    ...
    public boolean handleActivityEvent(MotionEvent activityEvent) {
        ...
        if (isMe()) {
            if (activityEvent.getAction() == MotionEvent.ACTION_UP) {
                if (this.getLeft() + this.getWidth() / 2 > ((FrameLayout) this.getParent().getParent()).getWidth() - this.getWidth() - DensityUtils.dip2px(mContext, 10)) {
                    //用户完成了选择动作
...
//              Log.v("yupeng", "yu" + lp.leftMargin);
                lp.leftMargin = (int) (activityEvent.getX() - ((FrameLayout) this.getParent().getParent()).getLeft()) - this.getWidth() / 2 + DensityUtils.dip2px(mContext, 5f);
